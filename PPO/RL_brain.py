...
class PolicyNet(nn.Module):  # 根据状态输出动作选择概率
...
class ValueNet(nn.Module):  # 根据状态输出决策评估的值
...
class PPO:
    def __init__(self, n_states, n_hiddens, n_actions,
                 actor_lr, critic_lr, lmbda, epochs, eps, gamma, device, writter):
        # 实例化策略网络
    ...
    def learn(self, transition_dict):
        ...
        for i in range(self.epochs):
            ...
            if i == self.epochs - 1:
                ...
                self.writter.add_scalars("loss", {"critic_loss": critic_loss.item()}, self.learn_index)
        self.learn_index += 1
